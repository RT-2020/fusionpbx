...
//get default port and player URL
	...
	$player_url = $settings->get('camera', 'player_url', '/camera-player/player');
...
// Camera player JavaScript
	...
	echo "	if (!playerBaseUrl) {\n";
	echo "		alert('未配置播放器地址');\n";
	echo "		return;\n";
	echo "	}\n";
	echo "	if (window.location.protocol === 'https:' && playerBaseUrl.toLowerCase().indexOf('http://') === 0) {\n";
	echo "		playerBaseUrl = 'https://' + playerBaseUrl.substring(7);\n";
	echo "	}\n";
	echo "	currentPlayerUrl = playerBaseUrl + '?url=' + rtspUrl;\n\n";
	echo "	if (!currentPlayerUrl) {\n";
	echo "		alert('未配置播放器地址');\n";
	echo "		return;\n";
	echo "	}\n";
