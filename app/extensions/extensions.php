...
//get total extension count
	...
	if (!empty($search)) {
		...
		$sql .= " or lower(extension_owner) like :search ";
...
//show the content
	...
	echo th_order_by('extension_owner', $text['label-extension_owner'], $order_by, $order, null, "class='hide-sm-dn'");
	...
	if (is_array($extensions) && @sizeof($extensions) != 0) {
		...
		foreach($extensions as $row) {
			...
			echo "	<td class='hide-sm-dn'>".escape($row['extension_owner'])."&nbsp;</td>\n";
			if (permission_exists("outbound_caller_id_name")) {
