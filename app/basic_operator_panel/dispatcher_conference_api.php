...
try {
    ...
    switch($action) {
        ...
        case 'end_conference':
            ...
            echo json_encode([
                'success' => $result,
                'error' => $result ? null : 'Failed to end inline conference because ESL is unavailable'
            ]);
            break;
...
function cleanConferenceRoomName($conference_room) {
    return preg_replace('/[^0-9A-Za-z_.:-]/', '', trim((string) $conference_room));
}

function cleanConferenceExtension($extension) {
    return preg_replace('/[^0-9A-Za-z_*#-]/', '', trim((string) $extension));
}

function conferenceInviteTimeoutSeconds() {
    return 20;
}

function buildConferenceOriginateDialString($conference_room, $domain_name, $extension, $caller_id_name, $caller_id_number, $auto_answer = false) {
    $conference_room = cleanConferenceRoomName($conference_room);
    $extension = cleanConferenceExtension($extension);
    $origination_uuid = uuid();
    $timeout = conferenceInviteTimeoutSeconds();

    $dial_parts = [
        "origination_uuid=" . $origination_uuid,
        "origination_caller_id_name='" . str_replace("'", "\\'", (string) $caller_id_name) . "'",
        "origination_caller_id_number=" . preg_replace('/[^0-9A-Za-z_.:-]/', '', (string) $caller_id_number),
        "originate_timeout=" . $timeout,
        "call_timeout=" . $timeout,
        "hangup_after_bridge=false",
        "ignore_early_media=true",
        "operator_panel_origin=conference_invite",
        "operator_panel_conference_room=" . $conference_room,
        "operator_panel_conference_role=" . ($auto_answer ? 'dispatcher' : 'member'),
        "sip_h_X-Conference-Room=" . $conference_room,
        "sip_h_X-Call-Scene=conference_invite",
    ];
    if ($auto_answer) {
        $dial_parts[] = "sip_auto_answer=true";
    }
...
function channelMatchesConferenceRoom(array $channel, $conference_room) {
    $conference_room = cleanConferenceRoomName($conference_room);
    if ($conference_room === '') {
        return false;
    }
    ...
    $candidate_fields = [
        'operator_panel_conference_room',
        'variable_operator_panel_conference_room',
        'sip_h_X-Conference-Room',
        'variable_sip_h_X-Conference-Room',
        'variable_sip_h_x_conference_room',
    ];
    foreach ($candidate_fields as $field) {
        if (cleanConferenceRoomName($channel[$field] ?? '') === $conference_room) {
            return true;
        }
    }

    $application_data = (string) ($channel['application_data'] ?? '');
    if ($application_data !== '' && strpos($application_data, 'conference(' . $conference_room . '@') !== false) {
        return true;
    }
...
function clearPendingConferenceInviteLegs($conference_room, array $exclude_uuids = []) {
    $conference_room = cleanConferenceRoomName($conference_room);
    if ($conference_room === '') {
    ...
    $channels = json_decode((string) $channels_result, true);
    $rows = [];
    ...
    $exclude_map = [];
    foreach ($exclude_uuids as $exclude_uuid) {
        $exclude_uuid = trim((string) $exclude_uuid);
        if ($exclude_uuid !== '') {
            $exclude_map[$exclude_uuid] = true;
        }
    }
    ...
    foreach ($rows as $row) {
        ...
        if ($uuid === '' || isset($exclude_map[$uuid])) {
            continue;
        }
        if (!channelMatchesConferenceRoom($row, $conference_room)) {
            continue;
        }

        $matched++;
        $response = event_socket::api('uuid_kill ' . $uuid);
        if ($response !== false && stripos((string) $response, '-ERR') === false) {
            $killed++;
        }
    }

    error_log("Cleared pending conference invite legs - room: $conference_room, matched: $matched, killed: $killed");
    return ['matched' => $matched, 'killed' => $killed];
}
...
function conferenceProfileUsesAutoRecord($profile_name = 'default') {
    $candidates = [];
    if (!empty($_SESSION['switch']['conf']['dir'])) {
        $candidates[] = rtrim($_SESSION['switch']['conf']['dir'], '/\\') . '/autoload_configs/conference.conf.xml';
    }
    $candidates[] = dirname(__DIR__) . '/switch/resources/conf/autoload_configs/conference.conf.xml';
    ...
    foreach ($candidates as $path) {
        ...
        try {
            ...
            if (!$xml || empty($xml->profiles)) {
                continue;
            }
            foreach ($xml->profiles->profile as $profile) {
                if ((string) ($profile['name'] ?? '') !== $profile_name) {
                    continue;
                }
                foreach ($profile->param as $param) {
                    if ((string) ($param['name'] ?? '') === 'auto-record' && trim((string) ($param['value'] ?? '')) !== '') {
                        return true;
                    }
                }
            }
            ...
            error_log('Failed to inspect conference auto-record config: ' . $throwable->getMessage());
        }
    }
...
function endConference($conference_room) {
    ...
    if ($esl && $esl->is_connected()) {
        ...
        $response = $esl->request($cmd);
        $cleanup_result = clearPendingConferenceInviteLegs($conference_room);
        
        error_log("End conference command: $cmd, response: $response");
        error_log("End conference cleanup - room: $conference_room, result: " . json_encode($cleanup_result, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES));
        error_log("Conference ended in inline mode: $conference_room");
        return true;
    }

    error_log("Failed to end inline conference because ESL is unavailable: $conference_room");
    return false;
}
