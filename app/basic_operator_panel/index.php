...
//获取当前用户的分机信息（用于自动SIP注册）
	...
	$user_extensions_list = array_values(array_unique($user_extensions_list));
	if (is_array($first_extension ?? null)) {
		foreach (['extension', 'user', 'destination', 'number_alias'] as $field) {
			$value = trim((string) ($first_extension[$field] ?? ''));
			if ($value !== '') {
				$user_extension = $value;
				break;
			}
		}
		...
		$extension_row = null;
		$extension_uuid = trim((string) ($first_extension['extension_uuid'] ?? ''));

		if ($domain_uuid !== '' && $extension_uuid !== '') {
			...
			$extension_row = $database->select($sql, $parameters, 'row');
			unset($sql, $parameters);
		}

		if (!is_array($extension_row) && $domain_uuid !== '') {
			$lookup_values = [];
			foreach (['extension', 'user', 'destination', 'number_alias'] as $field) {
				$value = trim((string) ($first_extension[$field] ?? ''));
				if ($value !== '') {
			...
			foreach ($lookup_values as $lookup_value) {
				$sql = "select extension, number_alias, password from v_extensions ";
				$sql .= "where domain_uuid = :domain_uuid ";
				$sql .= "and (extension = :lookup_value or number_alias = :lookup_value) ";
				$sql .= "limit 1";
				$parameters = [
					'domain_uuid' => $domain_uuid,
		...
		if (is_array($extension_row)) {
			$db_extension = trim((string) ($extension_row['extension'] ?? ''));
			if ($db_extension !== '') {
				$user_extension = $db_extension;
			}
			$extension_password = trim((string) ($extension_row['password'] ?? ''));
		}

		if ($extension_password === '') {
			foreach (['password', 'sip_password', 'extension_password'] as $field) {
				$value = trim((string) ($first_extension[$field] ?? ''));
				if ($value !== '') {
					$extension_password = $value;
					break;
				}
			}
		}
	}
	$current_user_sip_config = [
		'ws' => 'wss://' . $server_ip . ':7443',
		'uri' => $user_extension !== '' ? 'sip:' . $user_extension . '@' . $domain_name : '',
		'user' => $user_extension,
...
<script language="JavaScript" type="text/javascript" src="<?php echo PROJECT_PATH; ?>/app/basic_operator_panel/resources/jssip.min.js?v=<?php echo filemtime(__DIR__ . '/resources/jssip.min.js'); ?>"></script>
<script language="JavaScript" type="text/javascript" src="<?php echo PROJECT_PATH; ?>/app/basic_operator_panel/resources/dispatcher-utils.js?v=<?php echo filemtime(__DIR__ . '/resources/dispatcher-utils.js'); ?>"></script>
<script language="JavaScript" type="text/javascript" src="<?php echo PROJECT_PATH; ?>/app/basic_operator_panel/resources/jssip-client.js?v=<?php echo filemtime(__DIR__ . '/resources/jssip-client.js'); ?>"></script>
<script language="JavaScript" type="text/javascript" src="<?php echo PROJECT_PATH; ?>/app/basic_operator_panel/resources/dispatcher-control.js?v=<?php echo filemtime(__DIR__ . '/resources/dispatcher-control.js'); ?>"></script>
<script language="JavaScript" type="text/javascript" src="<?php echo PROJECT_PATH; ?>/app/basic_operator_panel/resources/dispatcher-logger.js?v=<?php echo filemtime(__DIR__ . '/resources/dispatcher-logger.js'); ?>"></script>
<script language="JavaScript" type="text/javascript" src="<?php echo PROJECT_PATH; ?>/app/basic_operator_panel/resources/emergency-audio.js?v=<?php echo filemtime(__DIR__ . '/resources/emergency-audio.js'); ?>"></script>
		...
		success: function(response) {
			if (response.success && response.data) {
				...
				response.data.forEach(function(extData) {
					...
					if (extData.extension_owner) {
						label += ' [使用人: ' + extData.extension_owner + ']';
					}
					option.text = label;
...
<style type="text/css">
    ...
    .dispatcher-status-inline { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
    .dispatcher-alarm-status-inline { display:flex; align-items:center; gap:8px; flex-wrap:wrap; margin-top:6px; font-size:12px; color:#495057; }
    .alarm-status-indicator { display:inline-block; width:10px; height:10px; border-radius:50%; background:#adb5bd; }
    .alarm-status-online { background:#28a745; }
    .alarm-status-warning { background:#ffc107; }
    .alarm-status-offline { background:#dc3545; }
    .alarm-status-unknown { background:#adb5bd; }
    .alarm-status-meta { color:#6c757d; }
	...
	<table width="100%" cellpadding="0" cellspacing="0">
		<tr>
			<td width="50%">
				...
				<div class="dispatcher-alarm-status-inline">
					<span id="alarm-status-indicator" class="alarm-status-indicator alarm-status-unknown"></span>
					<span id="alarm-status-text">声光报警器状态读取中...</span>
					<span id="alarm-status-meta" class="alarm-status-meta"></span>
					<?php if (permission_exists('operator_panel_alarm_test')): ?>
					<button id="alarm-test-btn" class="btn btn-sm" type="button" onclick="window.location.href='emergency_alarm_test.php'">Alarm Test</button>
					<?php endif; ?>
					<?php if (permission_exists('default_setting_edit')): ?>
					<button id="alarm-config-btn" class="btn btn-sm" type="button" onclick="openAlarmConfigModal()">报警器配置</button>
					<?php endif; ?>
				</div>
			</td>
...
<div id="dispatcher-entity-status-panel" class="dispatcher-entity-status-panel">
    <div class="dispatcher-entity-status-header">
        <div>
            <div class="dispatcher-entity-status-title">用户 / 中继状态</div>
            <div id="dispatcher-entity-status-updated" class="dispatcher-entity-status-updated">等待首帧状态</div>
        </div>
        <div class="dispatcher-entity-status-actions">
            <button class="btn btn-sm btn-secondary" onclick="dispatcherControl.refreshEntityStatus(true).catch(function(){})">刷新状态</button>
        </div>
    </div>
    <div id="dispatcher-entity-status-summary" class="dispatcher-entity-status-summary"></div>
    <div id="dispatcher-entity-status-warnings" class="dispatcher-entity-status-warnings"></div>
    <div class="dispatcher-entity-status-body">
        <div class="dispatcher-entity-section">
            <div class="dispatcher-entity-section-title">用户状态</div>
            <div id="dispatcher-user-status-grid" class="dispatcher-entity-grid"></div>
        </div>
        <div class="dispatcher-entity-section">
            <div class="dispatcher-entity-section-title">中继状态</div>
            <div id="dispatcher-trunk-status-grid" class="dispatcher-entity-grid dispatcher-trunk-grid"></div>
        </div>
    </div>
</div>
...
<?php if (permission_exists('default_setting_edit')): ?>
<div id="alarm-config-modal" class="dispatcher-modal" style="display: none;">
	<div class="modal-overlay" onclick="closeAlarmConfigModal()"></div>
	<div class="modal-content" style="max-width: 520px;">
		<div class="modal-header">
			<h3>报警器配置</h3>
			<button class="modal-close-btn" onclick="closeAlarmConfigModal()">×</button>
		</div>
		<div class="modal-body">
			<div class="form-group">
				<label>当前配置:</label>
				<div id="alarm-config-current" class="alert alert-info" style="padding: 10px; background: #e3f2fd; border-radius: 5px; font-size: 12px;">读取中...</div>
			</div>
			<div class="form-group">
				<label>报警器 IP / 主机名:</label>
				<input type="text" id="alarm-config-host" class="form-control" value="192.168.50.1" placeholder="192.168.50.1" />
			</div>
			<div class="form-group">
				<label>报警器端口:</label>
				<input type="number" id="alarm-config-port" class="form-control" value="9003" min="1" max="65535" />
			</div>
			<div class="form-group">
				<label>Socket Timeout (ms)</label>
				<input type="number" id="alarm-config-timeout" class="form-control" value="1200" min="100" max="60000" />
			</div>
			<div class="form-group">
				<label>Audio Folder / Track</label>
				<div style="display:flex; gap:10px;">
					<input type="number" id="alarm-config-audio-folder" class="form-control" value="1" min="0" max="255" />
					<input type="number" id="alarm-config-audio-track" class="form-control" value="1" min="0" max="255" />
				</div>
			</div>
			<div class="form-group">
				<label>Strobe Mode</label>
				<input type="number" id="alarm-config-strobe-mode" class="form-control" value="3" min="0" max="255" />
			</div>
			<div class="form-group">
				<label>闊抽噺 (0-30)</label>
				<input type="number" id="alarm-config-volume" class="form-control" value="30" min="0" max="30" />
			</div>
			<div class="alert alert-info" style="padding: 10px; background: #e3f2fd; border-radius: 5px; font-size: 12px; margin-top: 10px;">
				此面板保存的是全局共享的报警器配置，并会同时持久化运行服务使用的超时、音频目录、曲目、频闪模式和音量参数。
			</div>
		</div>
		<div class="modal-footer">
			<button class="btn btn-primary" onclick="saveAlarmConfig()">保存</button>
			<button class="btn btn-secondary" onclick="closeAlarmConfigModal()">取消</button>
		</div>
	</div>
</div>
<?php endif; ?>
...
var emergencyAudio;
var dispatcherAutoRegisterState = {
	inFlight: false,
	manualStop: false,
	retryTimer: null,
	attempts: 0,
...
// 当前用户SIP配置（从PHP动态获取）
var CURRENT_USER_SIP_CONFIG = <?php echo json_encode($current_user_sip_config, JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT); ?>;

// 默认SIP配置 - 提供多个备用账号（保留用于手动选择）
window.OPERATOR_PANEL_CONTEXT = window.OPERATOR_PANEL_CONTEXT || {};
window.OPERATOR_PANEL_CONTEXT.currentExtension = CURRENT_USER_SIP_CONFIG.user || '';
window.OPERATOR_PANEL_CONTEXT.sessionExtensions = <?php echo json_encode($user_extensions_list); ?>;
window.OPERATOR_PANEL_CONTEXT.tabUuid = (function() {
	var key = 'basic_operator_panel_tab_uuid';
	var value = '';
	try {
		if (window.sessionStorage) {
			value = sessionStorage.getItem(key) || '';
		}
		if (!value && window.crypto && typeof window.crypto.randomUUID === 'function') {
			value = window.crypto.randomUUID();
		}
		if (!value) {
			value = 'tab_' + Date.now() + '_' + Math.random().toString(36).slice(2, 12);
		}
		if (window.sessionStorage) {
...
function normalizeSipConfig(config) {
	config = config || {};
	var normalized = {
		ws: config.ws || config.wsServers || '',
		uri: config.uri || '',
		user: config.user || config.authUser || '',
		password: config.password || '',
		displayName: config.displayName || config.display_name || ''
	};

	if (!normalized.user && normalized.uri) {
		var uriMatch = normalized.uri.match(/^sip:([^@]+)/i);
		if (uriMatch && uriMatch[1]) {
			normalized.user = uriMatch[1];
		}
	}

	if (!normalized.displayName) {
		normalized.displayName = normalized.user || '';
	}

	return normalized;
}

function getCurrentSipDisplayUser() {
	if (dispatcherControl && dispatcherControl.config && dispatcherControl.config.authUser) {
		return $.trim(dispatcherControl.config.authUser);
	}
	var formUser = $.trim($('#sip-user').val() || '');
	if (formUser) {
		return formUser;
	}
	return $.trim(CURRENT_USER_SIP_CONFIG.user || DEFAULT_SIP_CONFIG.user || '');
}

function syncCurrentSipContext(config, options) {
	var normalized = normalizeSipConfig(config);
	if (!normalized.user) {
		return normalized;
	}

	CURRENT_USER_SIP_CONFIG = $.extend({}, CURRENT_USER_SIP_CONFIG, normalized);
	DEFAULT_SIP_CONFIG = CURRENT_USER_SIP_CONFIG.user ? CURRENT_USER_SIP_CONFIG : DEFAULT_SIP_ACCOUNTS[0];
	window.OPERATOR_PANEL_CONTEXT.currentExtension = normalized.user;

	if (dispatcherControl) {
		dispatcherControl.presenceExtension = normalized.user;
	}

	if (!(options && options.skipFormFill)) {
		initSipRegisterForm();
	}
...
function readStoredSipConfig(storageKey) {
	var rawValue = '';
	try {
		...
		return normalizeSipConfig(JSON.parse(rawValue));
	} catch (error) {
		console.warn('读取本地 SIP 配置失败:', storageKey, error);
		return null;
	}
}

function getAutoRegisterSipConfig() {
	var config = normalizeSipConfig(CURRENT_USER_SIP_CONFIG);
	var storedConfigs = [
		readStoredSipConfig('sip_config'),
		readStoredSipConfig('dispatcher_sip_config')
	];

	storedConfigs.forEach(function(savedConfig) {
		if (!savedConfig) {
			return;
		}
		var sameUser = !!(config.user && savedConfig.user && config.user === savedConfig.user);
		var sessionHasSavedUser = !config.user &&
			window.OPERATOR_PANEL_CONTEXT &&
			Array.isArray(window.OPERATOR_PANEL_CONTEXT.sessionExtensions) &&
			window.OPERATOR_PANEL_CONTEXT.sessionExtensions.indexOf(savedConfig.user) !== -1;
		var canUseSavedConfig = sameUser || sessionHasSavedUser;
		if (!canUseSavedConfig) {
			return;
		}
		if (!config.ws && savedConfig.ws) {
			config.ws = savedConfig.ws;
		}
		if (!config.uri && savedConfig.uri) {
			config.uri = savedConfig.uri;
		}
		if (!config.user && savedConfig.user) {
			config.user = savedConfig.user;
		}
		if (!config.password && savedConfig.password) {
			config.password = savedConfig.password;
		}
		if (!config.displayName && savedConfig.displayName) {
			config.displayName = savedConfig.displayName;
		}
	});

	if (!config.displayName) {
		config.displayName = config.user || '';
	}

	return config;
}

function hasCompleteSipConfig(config) {
	return !!(config && config.ws && config.uri && config.user && config.password);
}

function clearAutoRegisterRetryTimer() {
	if (dispatcherAutoRegisterState.retryTimer) {
		clearTimeout(dispatcherAutoRegisterState.retryTimer);
		dispatcherAutoRegisterState.retryTimer = null;
	}
}

function scheduleAutoRegisterSip(reason, delay) {
	if (dispatcherAutoRegisterState.manualStop) {
		return;
	}
	clearAutoRegisterRetryTimer();
	dispatcherAutoRegisterState.retryTimer = setTimeout(function() {
		attemptAutoRegisterSip(reason);
	}, delay || 0);
}

function updateAlarmHealthStatus(response) {
	var indicator = $('#alarm-status-indicator');
	var text = $('#alarm-status-text');
	var meta = $('#alarm-status-meta');
	var runtime = response && response.runtime ? response.runtime : null;
	var serviceState = runtime && runtime.service_state ? String(runtime.service_state).toLowerCase() : '';
	var deviceState = runtime && runtime.device_state ? String(runtime.device_state).toLowerCase() : '';
	var configuredEndpoint = (response && response.configured_endpoint) || ((response && response.device_host ? response.device_host : '192.168.50.1') + ':' + (response && response.device_port ? response.device_port : 9003));
	var endpoint = configuredEndpoint;
	if (serviceState === 'running' && runtime && runtime.device_endpoint) {
		endpoint = runtime.device_endpoint;
	}
	var activeCount = response && typeof response.active_alarm_count !== 'undefined' ? parseInt(response.active_alarm_count, 10) || 0 : 0;
	var lastError = runtime && runtime.last_error ? String(runtime.last_error) : '';
	var connectedDeviceStates = ['connected', 'online', 'idle', 'alarming', 'pending'];
	var indicatorClass = 'alarm-status-unknown';
	var statusText = '声光报警器状态未知';

	if (serviceState === 'running' && connectedDeviceStates.indexOf(deviceState) !== -1) {
		indicatorClass = activeCount > 0 ? 'alarm-status-warning' : 'alarm-status-online';
		statusText = (activeCount > 0 || deviceState === 'alarming') ? '声光报警器已连接，当前报警中' : '声光报警器已连接';
	}
	else if (serviceState === 'running') {
		indicatorClass = 'alarm-status-warning';
		statusText = '报警服务运行中，设备未连接';
	}
	else if (runtime) {
		indicatorClass = 'alarm-status-offline';
		statusText = '报警服务未运行';
	}
	else {
		indicatorClass = 'alarm-status-offline';
		statusText = '报警服务无运行状态';
	}

	if (serviceState === 'degraded') {
		indicatorClass = 'alarm-status-warning';
		statusText = '报警后台降级，可用直连通道';
	}

	indicator.removeClass('alarm-status-online alarm-status-warning alarm-status-offline alarm-status-unknown').addClass(indicatorClass);
	text.text(statusText);

	var metaParts = ['TCP ' + endpoint];
	if (serviceState) {
		metaParts.push('服务 ' + serviceState);
	}
	if (deviceState) {
		metaParts.push('设备 ' + deviceState);
	}
	if (configuredEndpoint && configuredEndpoint !== endpoint) {
		metaParts.push('配置 ' + configuredEndpoint);
	}
	metaParts.push('活动报警 ' + activeCount);
	if (runtime && runtime.last_heartbeat) {
		metaParts.push('心跳 ' + runtime.last_heartbeat);
	}
	if (lastError) {
		metaParts.push('错误 ' + lastError);
	}
	meta.text(metaParts.join(' | '));
}

function fetchAlarmHealthStatus() {
	return $.ajax({
		url: 'dispatcher_api.php?action=alarm_health',
		type: 'GET',
		dataType: 'json'
	}).done(function(response) {
		if (response && response.success) {
			updateAlarmHealthStatus(response);
		}
		else {
			updateAlarmHealthStatus(null);
		}
	}).fail(function() {
		updateAlarmHealthStatus(null);
	});
...
function startAlarmHealthPolling() {
	if (alarmStatusPollTimer) {
		clearInterval(alarmStatusPollTimer);
	}
	fetchAlarmHealthStatus();
	alarmStatusPollTimer = setInterval(function() {
		fetchAlarmHealthStatus();
	}, 10000);
}
...
$(document).ready(function() {
	...
	restoreSipStatus();
	scheduleAutoRegisterSip('document-ready', 400);
	startAlarmHealthPolling();
	...
	dispatcherControl.on('registered', function() {
		dispatcherAutoRegisterState.inFlight = false;
		dispatcherAutoRegisterState.attempts = 0;
		dispatcherAutoRegisterState.manualStop = false;
		clearAutoRegisterRetryTimer();
		syncCurrentSipContext(dispatcherControl.config, { skipFormFill: true });
		var extDisplay = getCurrentSipDisplayUser();
...
$(window).on('load', function() {
	scheduleAutoRegisterSip('window-load', 0);
});
		...
		try {
			...
			if (Date.now() - data.timestamp < 300000 && data.status === 'online') {
				...
				if (dispatcherControl && dispatcherControl.isRegistered && dispatcherControl.isRegistered()) {
					...
					scheduleAutoRegisterSip('restore-mismatch', 0);
					updateSipStatus('offline', '未注册');
					disableDispatcherFunctions();
				}
			} else {
...
function openAlarmConfigModal() {
	loadAlarmConfig();
	$('#alarm-config-modal').show();
}

function closeAlarmConfigModal() {
	$('#alarm-config-modal').hide();
}

function loadAlarmConfig() {
	return $.ajax({
		url: 'dispatcher_api.php?action=get_alarm_config',
		type: 'GET',
		dataType: 'json'
	}).done(function(response) {
		...
		$('#alarm-config-host').val(response.data.host || '192.168.50.1');
		$('#alarm-config-port').val(response.data.port || 9003);
		$('#alarm-config-timeout').val(response.data.timeout_ms != null ? response.data.timeout_ms : 1200);
		$('#alarm-config-audio-folder').val(response.data.audio_folder != null ? response.data.audio_folder : 1);
		$('#alarm-config-audio-track').val(response.data.audio_track != null ? response.data.audio_track : 1);
		$('#alarm-config-strobe-mode').val(response.data.strobe_mode != null ? response.data.strobe_mode : 3);
		$('#alarm-config-volume').val(response.data.audio_volume != null ? response.data.audio_volume : 30);
		$('#alarm-config-current').text(response.data.configured_endpoint || ((response.data.host || '') + ':' + (response.data.port || 9003)));
	}).fail(function() {
		$('#alarm-config-host').val($('#alarm-config-host').val() || '192.168.50.1');
		$('#alarm-config-port').val($('#alarm-config-port').val() || 9003);
		$('#alarm-config-current').text('读取失败');
	});
}

// 保存报警器配置
function saveAlarmConfig() {
	var host = $.trim($('#alarm-config-host').val() || '');
	var port = $.trim($('#alarm-config-port').val() || '');
	var timeoutMs = $.trim($('#alarm-config-timeout').val() || '1200');
	var audioFolder = $.trim($('#alarm-config-audio-folder').val() || '1');
	var audioTrack = $.trim($('#alarm-config-audio-track').val() || '1');
	var strobeMode = $.trim($('#alarm-config-strobe-mode').val() || '3');
	var audioVolume = $.trim($('#alarm-config-volume').val() || '30');
	if (!host || !port) {
		alert('请填写报警器 IP 和端口');
		return;
	}
	...
	$.ajax({
		...
		if (!(response && response.success)) {
			alert((response && response.error) || '保存失败');
			return;
		}

		var endpoint = response.data && response.data.configured_endpoint ? response.data.configured_endpoint : (host + ':' + port);
		var restart = response.service_restart || null;
		var restartOk = !!(restart && restart.success);
		var restartAction = restart && restart.action ? restart.action : 'restart';
		var restartMessage = restart && restart.message ? restart.message : '';

		$('#alarm-config-current').text(endpoint);
		$('#alarm-config-host').val(response.data && response.data.host ? response.data.host : host);
		$('#alarm-config-port').val(response.data && response.data.port ? response.data.port : port);
		$('#alarm-config-timeout').val(response.data && response.data.timeout_ms != null ? response.data.timeout_ms : timeoutMs);
		$('#alarm-config-audio-folder').val(response.data && response.data.audio_folder != null ? response.data.audio_folder : audioFolder);
		$('#alarm-config-audio-track').val(response.data && response.data.audio_track != null ? response.data.audio_track : audioTrack);
		$('#alarm-config-strobe-mode').val(response.data && response.data.strobe_mode != null ? response.data.strobe_mode : strobeMode);
		$('#alarm-config-volume').val(response.data && response.data.audio_volume != null ? response.data.audio_volume : audioVolume);
		closeAlarmConfigModal();
		loadAlarmConfig();
		fetchAlarmHealthStatus();

		if (restartOk) {
			alert('报警器配置已保存为 ' + endpoint + '，并已自动' + (restartAction === 'reload' ? '重载' : '重启') + ' emergency_alarm 服务。');
		}
		else {
			alert('报警器配置已保存为 ' + endpoint + '，但自动重启 emergency_alarm 服务失败。' + (restartMessage ? '\n\n' + restartMessage : ''));
		}
	}).fail(function(xhr) {
		alert('保存失败: ' + ((xhr && xhr.responseText) || '网络错误'));
	});
}
...
function doSipRegister() {
	...
	dispatcherAutoRegisterState.inFlight = true;
...
function doSipUnregister() {
	dispatcherAutoRegisterState.manualStop = true;
	dispatcherAutoRegisterState.inFlight = false;
	dispatcherAutoRegisterState.attempts = 0;
	clearAutoRegisterRetryTimer();
...
function autoRegisterSip() {
	return attemptAutoRegisterSip('legacy-call');
}

// 启用调度功能按钮
function attemptAutoRegisterSip(reason) {
	clearAutoRegisterRetryTimer();
	...
	if (!dispatcherControl || typeof dispatcherControl.register !== 'function') {
		scheduleAutoRegisterSip('dispatcher-not-ready', 500);
		return;
	}

	if (dispatcherControl.isRegistered && dispatcherControl.isRegistered()) {
		return;
	}

	if (dispatcherAutoRegisterState.inFlight) {
		return;
	}

	var sipConfig = getAutoRegisterSipConfig();
	if (!hasCompleteSipConfig(sipConfig)) {
		console.warn('自动注册所需 SIP 配置不完整:', sipConfig);
		updateSipStatus('offline', '未配置分机');
		return;
	}

	dispatcherAutoRegisterState.inFlight = true;
	dispatcherAutoRegisterState.attempts += 1;
	syncCurrentSipContext(sipConfig);

	console.log('开始自动注册SIP:', {
		user: sipConfig.user,
		reason: reason || 'direct',
		attempt: dispatcherAutoRegisterState.attempts
	});
	updateSipStatus('connecting', '自动连接中...');

	dispatcherControl.register({
		uri: sipConfig.uri,
		wsServers: sipConfig.ws,
		authUser: sipConfig.user,
		password: sipConfig.password,
		displayName: sipConfig.displayName
	})
		.then(function() {
			console.log('自动注册成功:', sipConfig.user);
		})
		.catch(function(error) {
			dispatcherAutoRegisterState.inFlight = false;
			console.error('自动注册失败:', error && error.message ? error.message : error);
			if (!dispatcherAutoRegisterState.manualStop && dispatcherAutoRegisterState.attempts < dispatcherAutoRegisterState.maxAttempts) {
				updateSipStatus('offline', '自动注册失败，正在重试');
				scheduleAutoRegisterSip('retry', Math.min(dispatcherAutoRegisterState.attempts * 1500, 5000));
			}
			else {
...
function initiateEmergencyToExtension(targetExt, operatorExt, emergencyUuid, callback) {
	...
	var url = 'exec.php?cmd=originate&source=' + operatorExt + '&destination=' + targetExt + '&emergency=true&emergency_uuid=' + encodeURIComponent(emergencyUuid || '');
...
<script type="text/javascript">
    ...
    function mute_call(uuid, action) {
		...
		function __dispatcherMonitorRequest(payload) {
			if (window.dispatcherControl && typeof dispatcherControl.requestMonitorCall === 'function') {
				try {
					if (typeof dispatcherControl.prepareMonitorAutoAnswer === 'function') {
						dispatcherControl.prepareMonitorAutoAnswer(payload && payload.mode);
					}
				} catch (e) {
					console.warn('prepare monitor auto answer failed:', e);
				}
				dispatcherControl.requestMonitorCall(payload)
					.then(function() {
						if (window.DispatcherUtils && DispatcherUtils.toast) {
							DispatcherUtils.toast(payload.mode === 'barge' ? '已发起强插请求' : '已发起监听请求', 'success');
						}
					})
					.catch(function(error) {
						console.error('monitor request failed:', error);
						try {
							if (typeof dispatcherControl.clearMonitorAutoAnswer === 'function') {
								dispatcherControl.clearMonitorAutoAnswer(payload && payload.mode);
							}
						} catch (clearError) {
							console.warn('clear monitor auto answer failed:', clearError);
						}
						if (window.DispatcherUtils && typeof DispatcherUtils.alert === 'function') {
			...
			if (window.DispatcherUtils && typeof DispatcherUtils.alert === 'function') {
				DispatcherUtils.alert('当前页面未加载新版监控能力', 'error');
			}
		}
		...
		function eavesdrop_call(ext, chan_uuid) {
			if (ext !== '' && chan_uuid !== '') {
				try {
					...
					console.warn('hangup barge sessions before eavesdrop failed:', e);
				}
				var operatorExt = (document.getElementById('eavesdrop_dest')) ? document.getElementById('eavesdrop_dest').value : '';
				__dispatcherMonitorRequest({
					mode: 'listen',
					targetExtension: ext,
					targetChannelUuid: chan_uuid,
					operatorExtension: operatorExt
				});
			}
		}
		...
		function three_way_call_with_ext(ext, chan_uuid, operator_ext) {
			...
			try {
				...
				console.warn('hangup eavesdrop sessions before barge failed:', e);
			}
			__dispatcherMonitorRequest({
				mode: 'barge',
				targetExtension: ext,
				targetChannelUuid: chan_uuid,
				operatorExtension: operator_ext
			});
		}

		function hangup_both(uuid_a, uuid_b) {
			var channelUuids = [];
			if (uuid_a) channelUuids.push(uuid_a);
			if (uuid_b && uuid_b !== uuid_a) channelUuids.push(uuid_b);
			if (window.dispatcherControl && typeof dispatcherControl.forceReleaseCall === 'function') {
				dispatcherControl.forceReleaseCall({ channelUuids: channelUuids })
					.then(function() {
						if (window.DispatcherUtils && DispatcherUtils.toast) {
							DispatcherUtils.toast('已发送强拆请求', 'success');
						}
					})
					.catch(function(error) {
			...
			if (window.DispatcherUtils && typeof DispatcherUtils.alert === 'function') {
				DispatcherUtils.alert('当前页面未加载新版强拆能力', 'error');
			}
		}
	</script>
